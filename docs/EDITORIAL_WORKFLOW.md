...
- **Section** says where the article belongs. Lab contains reproducible experiments and projects; Perspectives contains viewpoints, reading records, and shorter essays.
...
Topics must come from `src/data/topics.json`. Use one optional `primaryTopic`; it must also be present in `topics`. Use 2–5 tags. Perspectives articles should still receive professional topics when relevant so that viewpoints connect to Foundation, Research, Current, and Implementation material.
